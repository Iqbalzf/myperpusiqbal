...
if ($awal && $akhir) {
    $where .= " AND p.tanggal_kembali BETWEEN '$awal' AND '$akhir'";
}
...
/* ======================
   DATA PENGEMBALIAN + DENDA (LENGKAP)
====================== */
$data = $conn->query("
    SELECT 
        p.id_peminjaman,
...
<h4 class="mb-4 text-center">Laporan Pengembalian & Denda</h4>
...
<table class="table table-bordered table-hover">
    ...
    <tbody>
        <?php if ($data->num_rows > 0): while ($row = $data->fetch_assoc()):
                $terlambat = strtotime($row['tanggal_kembali']) > strtotime($row['tanggal_jatuh_tempo']);
        ?>
